...
class LiveBridge:
    def __init__(self, state: LiveRaceState, radio: LiveRadio) -> None:
        self.state = state
        self.radio = radio
...
async def _capture(
    microphone: PushToTalkMicrophone, state: LiveRaceState, radio: LiveRadio
...
async def live_dialogue(
    ...
    try:
        ...
        print(f"Radio ready. Hold {stt.ptt_key} when telemetry is ready; ESC or Ctrl+C exits.")
        print("Critical calls interrupt speech/capture. Release the key and ask again afterwards.")
        while True:
            try:
                capture = asyncio.create_task(_capture(microphone, state, radio))
                try:
...
async def voice_iracing(
    ...
    *,
    ...
    radio = LiveRadio(automatic, capacity=config.tts.queue_capacity)
    ...
    try:
        ...
        telemetry = asyncio.create_task(_read_iracing(config_path, limit, output, live=bridge))
        dialogue = asyncio.create_task(
            live_dialogue(config, stt, state, radio, recognizer, speaker, reply_language)
        )
