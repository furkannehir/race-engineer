...
def test_version_one_keyboard_setting_migrates_without_rewriting_file(tmp_path):
    ...
    settings = load_settings(path, PanelSettings())
    assert settings.version == 2
    assert settings.ptt_binding == legacy_binding("RALT")
    assert settings.volume == 61
...
def test_owned_server_stopped_after_cancel_during_startup(tmp_path, monkeypatch):
    ...
    async def run():
        ...
        await run_until_stopped(
            run_engineer(
                tmp_path, ROOT / "config/default.toml", AppConfig(), PanelSettings(), control
            ),
            control,
        )
...
def test_live_control_stops_all_components_and_passes_config(monkeypatch):
    closed = []
    config = PanelSettings(announce_position_changes=False).apply(AppConfig())
    control = LiveControl()
...
def test_engineer_stop_does_not_cancel_live_cleanup_a_second_time(monkeypatch):
    ...
    asyncio.run(
        run_engineer(
            ROOT, ROOT / "config/default.toml", AppConfig(), PanelSettings(), LiveControl()
        )
    )
